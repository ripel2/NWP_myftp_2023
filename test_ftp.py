...
class TestPwd1(Test):
    ...
    def run_test(self) -> bool:
        ...
        if not "is current directory" in data.decode('utf-8'):
            print("Test failed: PWD command did not return the correct message", file=sys.stderr)
            return False
        return True
...
class TestCwd2(Test):
    ...
    def run_test(self) -> bool:
        ...
        if not "/tests\"" in data.decode('utf-8'):
            print("Test failed: PWD command did not return the correct path", file=sys.stderr)
            return False
