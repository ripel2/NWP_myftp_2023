...
{
    if (!path_exists(path)) {
        LOG_ERROR("Client %d tried to delete a non-existing file", client->fd);
        client_printf(client, "%d %s.\r\n", 550, "File not found");
        return;
    }
    if (!path_is_file(path)) {
        LOG_ERROR("Client %d tried to delete a non-file", client->fd);
        client_printf(client, "%d %s.\r\n", 550, "Not a file");
        return;
    }
    if (remove(path) != 0) {
        LOG_ERROR("Client %d tried to delete a file he doesn't own",
        client->fd);
...
void dele_command(server_t *server, client_t *client)
{
    ...
    if (client->username == NULL) {
        LOG_WARNING("Client %d sent command before logging in", client->fd);
        client_printf(client, "%d %s.\r\n", 530, "Not logged in");
        return;
    }
    if (client->buffer[4] != ' ' ||
    strcspn(client->buffer + 5, " \n\r\t\v\f") == 0) {
        LOG_WARNING("Client %d sent invalid command", client->fd);
